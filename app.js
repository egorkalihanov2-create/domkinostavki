...
function renderSummary() {
  ...
  const cards = [
    ["Итоговый баланс", `${state.balance} бонусов`, ""],
...
function claimPromoBonus(event) {
...
function renderPromoBonus() {
  ...
  elements.promoBonus.setAttribute("aria-disabled", String(state.promoBonusClaimed));
...
function renderBetSlip(bets) {
  ...
  for (const bet of [...bets].reverse()) {
    ...
    card.innerHTML = `
      ...
      <span>${betLabel(bet)} · ${bet.stake} бонусов · x${bet.odds}</span>
